...
export default function TopAppBar() {
  return (
    <nav className="sticky top-0 z-10 w-full bg-white px-3 py-2 sm:px-4">
  ...
  return (
    <li {...props}>
      <NavigationMenuLink asChild>
        <Link
          href={href}
          className="block select-none rounded-md p-3 no-underline outline-hidden transition-colors hover:bg-muted focus:bg-muted"
        >
...
function MobileNavItem({ title, description, href, icon: Icon }) {
  return (
    <Link
      href={href}
      className="flex items-start gap-3 rounded-md px-2 py-2 transition-colors hover:bg-muted"
    >
      <div className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-fuchsia-50 text-fuchsia-700">
      ...
      <div className="min-w-0">
        ...
        <div className="line-clamp-2 text-xs text-gray-500">{description}</div>
      </div>
    </Link>
  );
}
