 ...
 * @param {React.ReactNode} [props.headerRight] - Optional element rendered on the right side of the header.
 * @param {React.ReactNode} [props.headerContent] - Optional custom header content replacing title/subtitle layout.
 * @param {React.ReactNode} [props.footerContent] - Optional footer content rendered after the section body.
 * @returns {JSX.Element} The rendered Section component.
 */
export default function Section({ children, icon, title, subtitle, className = "", iconClassName = "", headerRight = null, headerContent = null, footerContent = null }) {
  return (
    <section className={`p-5 mb-6 bg-white border border-gray-200 rounded-2xl sm:p-6 bg-white/80 dark:border-slate-800/80 dark:bg-slate-950/40 ${className}`}>
      {headerContent ? (
