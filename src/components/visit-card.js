    ...
    <div className="group space-y-2">
      ...
      >
        ...
        <div className="text-center">
          {visitLocation ? <div className={`text-[10px] font-semibold uppercase tracking-[0.3em] ${isLive ? "text-white/50 dark:text-slate-500" : "text-slate-500 dark:text-slate-400"}`}>Based in {visitLocation}</div> : null}
          ...
          {start ? <div className={`mt-2 text-sm ${isLive ? "text-white/50 dark:text-slate-500" : "text-slate-500 dark:text-slate-400"}`}>{isSameDay ? format(start, "MMM d") : start.getFullYear()}</div> : null}
        </div>
