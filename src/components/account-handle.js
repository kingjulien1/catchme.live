...
export default function AccountHandleClient({ username, name, profilePictureUrl, followersCount, accountType, mediaCount, bio, className = "" }) {
  ...
  return (
    <HoverCard openDelay={120} {...hoverCardProps}>
      ...
      <HoverCardContent className="relative z-50 w-[calc(100vw-3rem)] max-w-[520px] overflow-hidden rounded-[28px] border border-slate-200 bg-white p-4 sm:p-6 shadow-xl shadow-slate-200/60 dark:border-slate-800 dark:bg-slate-950 dark:shadow-black/40">
        ...
        <div className="relative z-10 flex items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-2 text-xs font-semibold pointer-events-none">
            ...
            {typeof followersCount === "number" ? (
              <span className="inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-4 py-1.5 text-emerald-700 dark:border-emerald-500/40 dark:bg-emerald-500/10 dark:text-emerald-200">Bookings open</span>
            ) : null}
          </div>
        ...
        <div className="relative z-10 mt-5 flex items-center gap-3 pointer-events-none">
          <Avatar className="h-8 w-8 border border-slate-200 bg-white text-slate-400 shadow-sm sm:h-10 sm:w-10 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300">
